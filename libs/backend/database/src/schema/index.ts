...
export const products = pgTable(
  ...
  (table) => ({
    ownerStatusIndex: index("products_owner_id_id_idx").on(table.ownerId, table.id),
  }),
);
...
export const walletAccounts = pgTable(
  ...
  (table) => ({
    ownerCurrencyUnique: uniqueIndex("wallet_accounts_owner_currency_idx").on(
      table.ownerId,
      table.currency,
    ),
  }),
...
export const orders = pgTable(
  ...
  (table) => ({
    ...
    idempotencyUnique: uniqueIndex("orders_buyer_idempotency_key_idx").on(
      table.buyerId,
...
export const ledgerEntries = pgTable(
  ...
  (table) => ({
    transactionIndex: index("ledger_entries_transaction_id_idx").on(table.transactionId),
  }),
);
