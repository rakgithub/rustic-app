...
app.post("/products", async (context) => {
  ...
  if (!input.success)
    return context.json({ error: "Invalid product", details: input.error.flatten() }, 400);
...
app.post("/dev/wallet/top-up", async (context) => {
  ...
  if (process.env["NODE_ENV"] === "production")
    return context.json({ error: "Not available in production" }, 404);
  ...
  return context.json(
    await topUpWallet(getDatabase().db, ownerId, input.data.amountMinor, input.data.currency),
    200,
  );
});
...
app.post("/products/:productId/purchase", async (context) => {
  ...
  try {
    return context.json(
      await purchaseProduct(getDatabase().db, ownerId, productId.data, input.data.idempotencyKey),
      201,
    );
  } catch (error) {
  ...
  const result = await handleUpload({
    ...
    onUploadCompleted: async ({ blob, tokenPayload }) => {
      const token = tokenPayload
        ? (JSON.parse(tokenPayload) as { ownerId: string; productId: string })
        : null;
...
app.openAPIRegistry.registerPath({
  ...
  responses: {
    200: {
      ...
      content: { "application/json": { schema: productWithImagesResponseSchema } },
    },
    404: {
...
app.openAPIRegistry.registerPath({
  ...
  responses: {
    ...
    400: {
      description: "Product is not ready",
      content: { "application/json": { schema: errorResponseSchema } },
    },
  },
});
...
app.openAPIRegistry.registerPath({
  ...
  responses: {
    200: {
      ...
      content: { "application/json": { schema: z.array(productWithImagesResponseSchema) } },
    },
  },
});
...
app.openAPIRegistry.registerPath({
  ...
  responses: {
    200: {
      description: "Vercel Blob client upload token",
...
app.openAPIRegistry.registerPath({
  ...
  responses: {
    200: {
      ...
      content: { "application/json": { schema: walletResponseSchema.nullable() } },
    },
  },
});
...
app.openAPIRegistry.registerPath({
  ...
  responses: {
    200: {
      ...
      content: { "application/json": { schema: z.array(walletTransactionResponseSchema) } },
    },
  },
});
...
app.openAPIRegistry.registerPath({
  method: "post",
  path: "/dev/wallet/top-up",
  request: {
    headers: z.object({ "x-user-id": z.string().min(1) }),
    body: { content: { "application/json": { schema: topUpSchema } } },
  },
  responses: {
    200: {
...
app.openAPIRegistry.registerPath({
  ...
  responses: {
    200: {
      ...
      content: { "application/json": { schema: z.array(orderResponseSchema) } },
    },
  },
});
...
app.openAPIRegistry.registerPath({
  ...
  path: "/orders/{orderId}",
  request: {
    params: z.object({ orderId: productIdSchema }),
    headers: z.object({ "x-user-id": z.string().min(1) }),
  },
  responses: {
    200: {
